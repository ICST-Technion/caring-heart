...
List<List<double>> ComputeDistanceMatrix(int pointsAmount, List<Point<double>> map) {
...
List<Point<double>> routePlanningEngine(List<Point<double>> map, int k) {
  ...
  var routePointsIndexes = FindMinCluster(distanceMatrix, k);
...
double ComputeClusterDistanceFactor(
...
List<int> FindMinCluster(List<List<double>> graph, int k) {
  // Initiate min value
  ...
  for (int i = 0; i < graph.length; i++) {
    ...
    double clusterDistanceFactor =
        ComputeClusterDistanceFactor(kClosestNodes, graph);
