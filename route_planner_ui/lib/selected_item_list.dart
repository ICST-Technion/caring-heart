...
class SelectedList {
  ...
  Widget SelectedItemList(bool draggable) {
    if (!Logic.getProvider(context, false).isSelectedEmpty()) {
      return SizedBox(
        height: Logic.ScreenSize(context).height / 2.4,
        child: ReorderableList(
            shrinkWrap: true,
            itemBuilder: (context, idx) => getSelectedItem(
                Logic.getProvider(context, true).selectedItems[idx].item,
                idx,
                draggable),
            itemCount: Logic.getProvider(context, true).selectedItems.length,
            onReorder: (prev, current) {
              if (current > prev) {
                current = current - 1;
              }
              Logic.getProvider(context, false)
  ...
  Widget SelectTimeBtn(int idx) {
    List<String> options = [];
    String time =
        Logic.getProvider(context, true).selectedItems[idx].pickupTime;
    for (int h = 8; h <= 18; h++) {
    ...
    return Container(
      ...
      child: DropdownButtonFormField(
        ...
        onChanged: (String? value) {
          Logic.getProvider(context, false).changePickupTimeAt(idx, value!);
        },
      ),
    );
  }
